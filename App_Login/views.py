...
def user_change(request):
    ...
    if request.method == 'POST':
        form = UserProfileChange(request.POST, instance=current_user)#request.POST er value gulo dea instance er value replace korlm
        if form.is_valid():
            form.save()
            form = UserProfileChange(instance=current_user)#16:00 again load korlm notun info gulo, na hoe puraton info gulai dekhabe
